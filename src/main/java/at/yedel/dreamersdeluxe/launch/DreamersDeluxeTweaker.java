...
public class DreamersDeluxeTweaker implements ITweaker {
    ...
    public void acceptOptions(List<String> args, File gameDir, File assetsDir, String profile) {
        ...
        if ((Boolean) Launch.blackboard.get("fml.deobfuscatedEnvironment") == false) {
            queueTweaker("Hypixel Mod API", "at.yedel.dreamersdeluxe.launch.HypixelModAPITweaker");
            handleLocalModAPI(gameDir);
            queueTweaker("Mod Requeue", ModRequeueTweaker.class.getName());
        }
        else {
            DIARY.info("In development environment, skipping Mod API and mod requeue tweakers!");
        }
    }
    ...
    private void handleLocalModAPI(File gameDir) {
        ...
        for (File modFile : allModFiles) {
            try (JarFile modJar = new JarFile(modFile)) {
                ...
                DIARY.fatal("Couldn't instantiate JarFile from mod file! Local copy compatibility will not work.");
            }
        }
        if (modApiFile != null) {
            DIARY.info("Found local copy of Hypixel Mod API with version {}, loading local copy compatibility...", modApiVersion);
    ...
    private void queueTweaker(String name, String className) {
        // example: name = Hypixel Mod API -> propertyName = "dreamersdeluxe.launch.hypixel-mod-api"
        String propertyName = "dreamersdeluxe.launch." + name.toLowerCase().replace(" ", "-");
        String propertyValue = System.getProperty(propertyName);
        if (!Objects.equals(propertyValue, "false")) {
            DIARY.info("Queueing {} tweaker {}", name, className);
            TweakClasses.add(className);
        }
        else {
            DIARY.warn("Skipping queueing of {} tweaker {}, property {} = false!", name, className, propertyName);
        }
    }
